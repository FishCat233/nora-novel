...
* [ ] Task 1: 创建快照存储模块 `storage/snapshot.py`
  ...
  * [ ] SubTask 1.4: 实现 `list_snapshots()` 列出所有用户存档（排除.auto\_save.json）
...
* [ ] Task 5: 修改 `main.py` 初始化快照存储和处理读档
  ...
  * [ ] SubTask 5.2: 在 session\_state 初始化时创建 snapshot\_storage
...
* Task 3 依赖 Task 1（需要 get\_auto\_save 方法）

* Task 4 依赖 Task 1（需要 auto\_save 方法）
